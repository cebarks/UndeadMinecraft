...
public class EntitySurvivor extends EntityWithInventory implements IMob{
	...
	public EntitySurvivor(World world, boolean debugMode, String second) {
		...
		tasks.addTask(1, new EntityAIAttackOnCollide(this,
				net.undead.entity.EntityNewZombie.class, moveSpeed, false));
		tasks.addTask(2, new EntityAIAttackOnCollide(this,
				net.minecraft.entity.monster.EntityMob.class, moveSpeed, true));
		tasks.addTask(3, new EntityAIWander((net.minecraft.entity.EntityCreature)this, moveSpeed));
		tasks.addTask(4, new EntityAIWatchClosest(this,
				EntityNewZombie.class, 8F));
		tasks.addTask(5, new EntityAILookIdle(this));
		targetTasks.addTask(1, new EntityAIHurtByTarget(this, false));
		targetTasks.addTask(2, new EntityAINearestAttackableTarget(this,
				EntityNewZombie.class, 16F, 0, true));
		targetTasks.addTask(2, new EntityAINearestAttackableTarget(this,
				net.minecraft.entity.monster.EntityMob.class, 16F, 0, false));
		isDebug = debugMode;
		firstName = pickRandomName(1);
		if (second == null) {
			secondName = pickRandomName(2);
	...
	public void followPlayer() {
		...
		if (player != null) {
			float f = player.getDistanceToEntity(this);
			if (f > 4F && f < 16F) {
				pathentity = worldObj.getPathEntityToEntity(this, player, 16F,
	...
	public boolean isItemValuable(Item item) {
		Item valuable[] = { Item.swordWood, Item.swordStone, Item.swordSteel,
				Item.swordGold, Item.swordGold, Item.swordDiamond,
				Item.diamond, Item.redstone, Item.bed, Item.cookie,
				Item.porkCooked, Item.porkRaw, Item.appleGold,
				Item.chickenCooked, Item.chickenRaw, Item.arrow, Item.bow,
				Item.axeSteel, Item.axeDiamond, Item.axeGold, Item.bootsGold,
				Item.bootsSteel, Item.bootsLeather, Item.legsChain,
				Item.legsDiamond, Item.legsGold, Item.legsSteel,
				Item.legsLeather, Item.bucketLava, Item.bucketMilk,
				Item.bucketEmpty, Item.boat, Item.minecartCrate,
				Item.minecartEmpty, Item.minecartPowered, Item.egg,
				Item.rottenFlesh, Item.goldNugget, Item.ingotGold,
				Item.ingotIron, Item.plateChain, Item.plateDiamond,
				Item.plateGold, Item.plateLeather, Item.plateSteel,
				UndeadMinecraft.clothes };
		for (Item i : valuable) {
			if (i.itemID == item.itemID) {
				return true;
	...
	public void pickupItems() {
			@SuppressWarnings("unchecked")
			ArrayList<EntityItem> items = (ArrayList<EntityItem>) worldObj
				.getEntitiesWithinAABB(net.minecraft.entity.item.EntityItem.class,
						boundingBox.expand(16D, 32D, 16D));
		
		if (items.isEmpty())
			return;
		for (EntityItem item : items) {
			if (isItemValuable(item.getEntityItem().itemID)) {
				setPathToEntity(worldObj.getPathEntityToEntity(this, item, 16F,
						true, false, false, true));
			}
			if (item.getDistanceToEntity(this) < 2F) {
				inventory.addItemStackToInventory(item.getEntityItem());
	...
	public String getRandomSkinLocation(int i) {
		...
		if (i == 1) {
			return "/textures/ZombieMod/Survivors/blankChar"
					+ skinColour[rand.nextInt(skinColour.length)] + ".png";
		}
		if (i == 2) {
			return "/textures/ZombieMod/Survivors/Eyes/eyes"
					+ eyeColour[rand.nextInt(eyeColour.length)] + ".png";
		}
		if (i == 3) {
			return "/textures/ZombieMod/Survivors/Hair/hair"
					+ hairColour[rand.nextInt(hairColour.length)] + ".png";
		}
		if (i == 4) {
			return "/textures/ZombieMod/Survivors/Shirts/shirt"
					+ shirtColour[rand.nextInt(shirtColour.length)] + ".png";
		}
		if (i == 5) {
			return "/textures/ZombieMod/Survivors/Pants/pants"
					+ pantsColour[rand.nextInt(pantsColour.length)] + ".png";
		}
		if (i == 6) {
			return "/textures/ZombieMod/Survivors/Shoes/shoes"
					+ shoesColour[rand.nextInt(shoesColour.length)] + ".png";
		}
		return null;
	}
		...
		if (follower && getLeader() != null) {
			...
			if (survivor.getDistanceToEntity(this) > 4F) {
				getNavigator().setPath(
						worldObj.getPathEntityToEntity(this, survivor, 16F,
								true, false, false, true), moveSpeed);
			}
		}
	}
	...
	@SuppressWarnings("unchecked")
	public void attackZombies() {
		ArrayList<EntityNewZombie> zombies = (ArrayList<EntityNewZombie>) worldObj
				.getEntitiesWithinAABB(
						EntityNewZombie.class,
						AxisAlignedBB.getBoundingBox(posX, posY, posZ,
	...
	public void becomeZombie() {
		EntityNewZombie zombie = new EntityNewZombie(worldObj);
		zombie.setLocationAndAngles(posX, posY, posZ, rotationPitch,
				rotationYaw);
		worldObj.spawnEntityInWorld(zombie);
		setDead();
	}
	...
	public EntitySurvivor getLeader() {
		...
		List list = worldObj.getEntitiesWithinAABB(
				EntitySurvivor.class,
				AxisAlignedBB.getBoundingBox(posX, posY, posZ,
						posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(32D, 8D,
						32D));
		if (!list.isEmpty()) {
			for (int i = 0; i < list.size(); i++) {
				EntitySurvivor survivor = (EntitySurvivor) list.get(i);
	...
	protected void attackNormal(Entity entity, float distance, boolean weapon) {
		if (distance < 3.5F && attackTime-- <= 0) {
			...
			Minecraft mc = ModLoader.getMinecraftInstance();
			if (!onGround)
				mc.effectRenderer.addEffect(new EntityCrit2FX(mc.theWorld,
						entity));
			attackEntityAsMob(entity);
			attackTime = 30;
		}
	...
	protected void attackBow(Entity entity, float distance) {
		if (distance < 15F && attackTime-- <= 0
			...
			EntityArrow entityarrow = new EntityArrow(worldObj, this,
					(EntityLiving) entityToAttack, 1.6F, 12F);
			inventory.consumeInventoryItem(Item.arrow.itemID);
			worldObj.playSoundAtEntity(this, "random.bow", 1.0F, 1.0F / (this
					.getRNG().nextFloat() * 0.4F + 0.8F));
			if (distance < 5F)
	...
	protected void attackEntity(Entity entity, float f) {
		...
		if (getHeldItem() != null
				&& getHeldItem().itemID == Item.bow.itemID) {
			attackBow(entity, f);
		}
	}
	...
	public boolean attackEntityAsMob(Entity entity) {
		...
		swingItem();
		entity.addVelocity(-MathHelper.sin((rotationYaw * 3.141593F) / 180F)
				* 2 * 0.5F, 0.10000000000000001D,
				MathHelper.cos((rotationYaw * 3.141593F) / 180F) * 2
						* 0.5F);
		motionX *= 0.59999999999999998D;
		motionZ *= 0.59999999999999998D;
		setSprinting(false);
	...
	public void talk() {
		...
		if (attacker instanceof EntityNewZombie) {
			int distance = MathHelper.floor_double(attacker
					.getDistanceToEntity(this) + 1) / 10;
			String zombieSpeech[] = {
					"Zombies! about " + distance + " block"
							+ (distance > 1 ? "s " : " ") + " away!",
					"I can see a walker!",
	...
	public String pickRandomName(int part) {
		String namePart1[] = { "Shaun", "Ruben", "Jonathan", "Anten", "Yonathan",
				"Jack", "Jake", "Simon", "Alex", "Oliver", "Nathan",
				"Nathaniel", "Charlie", "Michael", "Harry", "Connor", "Scott",
				"Robert", "Aaron", "Adam", "Lee", "Philip", "Daniel", "John",
				"Nick", "Samuel", "Luke", "James", "Liam", "Callum", "Eddie",
				"Joe", "Kieron", "Andie", "Andy", "Anthony", "Stewart",
				"Benjamin", "Brandon", "Peter", "Rodney", "Cameron", "Derren",
				"Darren", "Owen", "Bradley", "Curtis", "Darcy", "Daryl",
				"Declan", "Richard", "Dillon", "Ed", "Edd", "Evan", "Gavin",
				"Gaz", "Craig", "Jamie", "Chris", "Sebastian", "Joshua",
				"Albert", "Hayden", "Patrick", "Homer", "Marge", "Lisa",
				"Bart", "Jim", "Lewis", "Ben", };
		String namePart2[] = { "Lawton", "Pickup", "Mullins", "Wilson",
				"Williams", "Martin", "Heaney", "Harrison", "Wild", "Richards",
				"McGrath", "Maguire", "Clayton", "Manning", "Murphy", "Rogers",
				"Sid", "Ward", "Watkinson", "Ireland", "Crossley", "Lloyd",
				"Bailey", "Kennedy", "Neilson", "Davies", "Crompton", "Smith",
				"Brown", "Green", "Ogden", "Jordan", "Roberts", "Walkley",
				"Drysdale", "Ashworth", "Wesker", "Hicks", "Robinson",
				"Rogersen", "Denham", "Tomlinson", "Monks", "Large", "Buckley",
				"Jones", "Jackson", "Bell", "Blaire", "Butterworth",
				"Cosgrove", "Anderson", "Simpson" };
		if (part == 1) {
			return namePart1[rand.nextInt(namePart1.length)];
	...
	@SuppressWarnings("unchecked")
	public boolean hasFamily() {
		List<EntitySurvivor> list = worldObj.getEntitiesWithinAABB(
				EntitySurvivor.class,
				AxisAlignedBB.getBoundingBox(posX, posY, posZ,
						posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(32D, 8D,
						32D));
	...
	public ItemStack getRandomStarter() {
		...
		case 11:
			return new ItemStack(Block.cobblestone, rand.nextInt(64));
		case 12:
			return new ItemStack(Block.sapling, rand.nextInt(16),
					rand.nextInt(3));
		case 13:
			return new ItemStack(Item.appleRed, rand.nextInt(16));
		case 14:
	...
	public ItemStack heldItem;
	public ItemStack defaultHeldItem;
	public EntityPlayer thePlayer = ModLoader.getMinecraftInstance().thePlayer;
	public int shoutTime;
	public EntityPlayer friend;
	public int pickupTime;
	public String familygreet[] = { "" };
>>>>>>> fad710903d9fb7a08307b123b3ef1d4be8058d44
